...
export interface Taxonomy extends RevisableBase {
  type: typeof TYPE;
  /** Authored topic and area classifications may be independent. */
...
export function rules(taxonomy: Taxonomy, _ledger: Ledger): string[] {
  ...
  for (const topic of taxonomy.topics) {
    ...
    if (topic.areaId === null) {
      if (taxonomy.independentTopics !== true) errors.push(`topic ${topic.id} needs an area unless independentTopics is true`);
    } else if (!areaIds.has(topic.areaId)) errors.push(`topic ${topic.id} names unknown area ${topic.areaId}`);
  }
  return errors;
}
