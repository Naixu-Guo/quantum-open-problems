...
| `GET /api/v1/problems?view=research&maxBytes=65536&…` | The same filters return complete research-view problems in a `qop-search-research/1` envelope. Whole-problem pagination obeys both row and compact UTF-8 JSON byte limits; `responseBytes` measures the actual response body. `maxBytes` accepts 16384–1048576 and excludes HTTP/MCP framing and token limits. A first problem that cannot fit returns 413 `response_budget_too_small` with `minimumRequiredBytes` and `problemId`. Cursors bind the view; change views with a fresh query. Default/explicit `summary` retains `qop-search/2` and rejects `maxBytes` |
