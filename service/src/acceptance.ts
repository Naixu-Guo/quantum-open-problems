...
export function consequences(context: AcceptanceContext, contribution: Contribution, verdict: Verdict): NewRecord[] {
  ...
  for (const problemId of touched) {
    ...
    if (auxiliary && settled && status !== "solved" && status !== "refuted") {
      ...
      out.push(decision(context, { kind: "status", targetType: "problem", targetId: problemId, status: refuted ? "refuted" : "solved", reviewIds: evidence, contributionIds: [contribution.id], ...supersede(previous) },
        refuted ? "A clause is refuted by an accepted claim." : "Every clause is resolved by accepted claims."));
    } else if (status === "open" && outcomes.some((o) => o !== "open")) {
      const evidence = statusEvidence(context, contribution, policy.thresholds["aiVerified"] ?? {}, VERIFIED_OR_PARTIAL, verdict);
      if (evidence === null) continue;
      out.push(decision(context, { kind: "status", targetType: "problem", targetId: problemId, status: "partial", reviewIds: evidence, contributionIds: [contribution.id], ...supersede(previous) },
