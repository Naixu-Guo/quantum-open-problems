...
const TOOLS: Tool[] = [
  ...
  { name: "get_contribution_status", description: "A contribution with its reviews, decisions, claims, derived state, and verification level.", inputSchema: { type: "object", required: ["contributionId"], properties: { contributionId: S.ulid } }, call: (a) => http("GET", `/api/v1/contributions/${str(a, "contributionId")}`) },
  { name: "get_record", description: "Any record's current revision by id.", inputSchema: { type: "object", required: ["id"], properties: { id: S.ulid } }, call: (a) => http("GET", `/api/v1/records/${str(a, "id")}`) },
  { name: "claim_queue_item", description: "For verifier and triage agents: the oldest contribution waiting for review that is not the caller's own and that the caller has not reviewed, with the review packet fields.", inputSchema: { type: "object", properties: {} }, call: async () => {
    ...
    if (!next?.id) return { status: result.status, body: { queue: "review", item: null, remaining: 0 } };
    const packet = await http("GET", `/api/v1/contributions/${next.id}`);
    return { status: packet.status, body: { queue: "review", item: packet.body, remaining: items.length - 1 } };
  } },
  // Work
  ...
  { name: "submit_review", description: "File one review of a contribution: kind (triage, verification, audit), independence and conflict-of-interest declarations, at least one mechanical method, checks, and a verdict.", inputSchema: { type: "object", required: ["contributionId", "kind", "independence", "conflictOfInterest", "methods", "checks", "verdict", "body"], properties: { contributionId: S.ulid, kind: { enum: ["triage", "verification", "audit"] }, independence: { type: "object" }, conflictOfInterest: { type: "object" }, methods: { type: "array", items: { type: "string" } }, checks: { type: "array" }, verdict: { type: "string" }, body: { type: "string" }, trajectoryId: { type: ["string", "null"] } } }, call: (a) => http("POST", "/api/v1/batches", { body: { message: `Review of ${str(a, "contributionId")}`, records: [{ type: "Review", ...a, trajectoryId: a["trajectoryId"] ?? null }] } }) },
  ...
  { name: "withdraw_contribution", description: "Withdraw your own submitted contribution.", inputSchema: { type: "object", required: ["contributionId", "reason"], properties: { contributionId: S.ulid, reason: { type: "string" } } }, call: (a) => http("POST", `/api/v1/contributions/${str(a, "contributionId")}/withdraw`, { body: { reason: str(a, "reason") } }) },
];
...
async function readResource(uri: string): Promise<{ status: number; body: unknown }> {
  ...
  if (problem) return http("GET", `/api/v1/problems/${encodeURIComponent(problem[1]!)}${problem[2] ? `/${problem[2]}` : ""}`);
  const contribution = route.match(/^contributions\/([^/]+)$/u);
  if (contribution) return http("GET", `/api/v1/contributions/${contribution[1]}`);
  const record = route.match(/^records\/([^/]+)$/u);
  if (record) return http("GET", `/api/v1/records/${record[1]}`);
  ...
  try {
    switch (method) {
      ...
      case "tools/call": {
        ...
        respond(id, { content: [{ type: "text", text: typeof result.body === "string" ? result.body : JSON.stringify(result.body, null, 1) }], isError });
        return;
      }
      case "resources/list":
