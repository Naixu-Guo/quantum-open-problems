...
export const REQUIRED_PROBLEM_READ_VERSION = "qop-problem-read/1";
export const PROBLEM_READ_UPGRADE_MESSAGE = `Category-based problem reading requires API contract ${REQUIRED_PROBLEM_READ_VERSION}. Deploy the matching API, run npm --prefix mcp run check:service, then activate MCP. Other compatible read tools remain available.`;
...
export function createAdapter(serviceUrl: string, apiKey: string | null = null, optionsReadOnly = false) {
  ...
  const TOOLS: Tool[] = [
    ...
    { readOnly: true, name: "search_problems", description: "Search scientific content with relevance ranking, controlled spelling/acronym variants and Chinese search terms. Full identifiers resolve separately. view:summary (default) returns summaries; view:research returns multiple complete problem research views for the same query, including full statements, research history and references. Research pages fit whole problems within maxBytes (default 32768 UTF-8 bytes of the entire API JSON response, excluding MCP framing); no problem text is truncated. If the first problem cannot fit, response_budget_too_small reports minimumRequiredBytes and problemId. Default sort is relevance with text, otherwise catalog edit time (not solution date). Repeat filters, view and sort with nextCursor; limit and maxBytes may change. If the catalog changed, start again. total counts all matches; count is this page. Difficulty may be unrated; never infer an objective difficulty score from ordering. Catalog edit dates are not scientific resolution dates. Use read_problem to request statement, history, references or comment from one result; host truncation can hide content even when the API page is complete.", inputSchema: { type: "object", dependentSchemas: { maxBytes: { required: ["view"], properties: { view: { const: "research" } } } }, properties: { ...problemFilters, view: { enum: ["summary", "research"], default: "summary", description: "Choose summaries or complete research views; tool combinations depend on the task." }, maxBytes: { type: "integer", minimum: 16384, maximum: 1048576, description: "Only with view:research. Entire API JSON response budget in UTF-8 bytes; API default is 32768. Excludes HTTP headers, MCP framing and token accounting." }, limit: { type: "integer", minimum: 1, maximum: 200, default: 50 }, offset: { type: "integer", minimum: 0, description: "Legacy pagination; prefer cursor for change detection." }, cursor: { type: "string", minLength: 1, description: "nextCursor from the previous page; repeat filters, view and sort, omit offset. limit and maxBytes may change." }, sort: { enum: ["relevance", "edited", "title", "stale"], description: "edited: catalog TeX edit time; stale: service human-review age. Neither is the date a scientific problem was solved." } } }, call: async (a) => {
    ...
    { readOnly: true, name: "read_problem", description: "Read one semantic category: statement (formal statement and any additional body), history (source and progress), references (authored bibliography and service references), or comment (maintainer comment, discussion and decisions). Initial default is statement. Every category includes status and statusSource copied from get_problem. content.status is the authoritative scientific problem status (Unsolved or Solved); statement.clauses[].status is service evidence state and may remain open for a Solved catalog problem. A category that fits returns format:json with the complete native object in content. Only an oversized category uses format:json-continuation with serialized JSON in text, including the same status and statusSource; follow nextCursor in order and concatenate text before parsing the category if machine parsing is needed. continued means earlier text exists; complete and nextCursor:null mean this category is finished. Keep the same id; omitted section inherits the cursor's category, and maxBytes may change. documentVersion detects document changes; on document_changed restart the category without combining revisions. maxBytes defaults to 8192 API JSON bytes (2048–65536), excluding MCP framing and tokens. Host output may still be truncated; reduce maxBytes if content is missing. researchContext distinguishes authored summaries from service-native records without such material. This reads catalog material, not cited papers' full text.", inputSchema: { type: "object", required: ["id"], properties: {
      id: { ...S.id, minLength: 1 },
      section: { enum: ["statement", "history", "references", "comment"], description: "Initial default is statement; omit with cursor to inherit its category." },
      documentVersion: { type: "string", minLength: 64, maxLength: 64, pattern: "^[0-9a-f]{64}$", description: "Document digest returned by read_problem. A changed document returns document_changed; restart instead of combining revisions." },
      cursor: { type: "string", minLength: 1, description: "Opaque nextCursor from the previous page. Keep id; section may be omitted to inherit its category. maxBytes may change." },
      maxBytes: { type: "integer", minimum: 2048, maximum: 65536, default: 8192, description: "Entire compact API JSON response budget in UTF-8 bytes; excludes HTTP headers, MCP framing and tokens." },
    } }, call: async (a) => {
      const { id: _id, ...options } = a;
      const result = await http("GET", `/api/v1/problems/${seg(str(a, "id"))}/read${query(options, true)}`);
      if (result.status < 400 && (result.body as Json)?.["schemaVersion"] !== REQUIRED_PROBLEM_READ_VERSION) {
        throw new AdapterError("INCOMPATIBLE_SERVICE", PROBLEM_READ_UPGRADE_MESSAGE, { httpStatus: result.status, retryable: false, ...(result.headers?.requestId ? { requestId: result.headers.requestId } : {}) });
      }
      return result;
    } },
