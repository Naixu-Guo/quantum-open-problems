...
export const SERVER_INFO = { name: "quantum-open-problems", version: "1.1.0" };
...
export function createAdapter(serviceUrl: string, apiKey: string | null = null, optionsReadOnly = false) {
  ...
  const TOOLS: Tool[] = [
    // Read
    { readOnly: true, name: "get_status", description: "Release date, ledger lastSequence, record counts, published problems by status. Poll this before anything else.", inputSchema: { type: "object", properties: {} }, call: () => http("GET", "/api/v1/status") },
    ...
    { readOnly: true, name: "search_problems", description: "Indexed problems (primary and promoted auxiliary) filtered by area, topic, status, difficulty, or text; sort=stale gives the maintenance backlog.", inputSchema: { type: "object", properties: { area: { type: "string" }, topic: { type: "string" }, status: { enum: ["Unsolved", "Solved"] }, difficulty: { type: "string" }, text: { type: "string" }, limit: { type: "integer", minimum: 1, maximum: 200 }, sort: { enum: ["title", "stale"] }, includeCandidates: { type: "boolean" } } }, call: (a) => http("GET", `/api/v1/problems${query(a)}`) },
    { readOnly: true, name: "get_problem", description: "A problem with its current statement, clause statuses, references with notes, comments, and decision chain.", inputSchema: { type: "object", required: ["id"], properties: { id: S.id } }, call: (a) => http("GET", `/api/v1/problems/${encodeURIComponent(str(a, "id"))}`) },
