...
export function createAdapter(serviceUrl: string, apiKey: string | null = null, optionsReadOnly = false) {
  ...
  const TOOLS: Tool[] = [
    ...
    { readOnly: true, name: "search_problems", description: "Search scientific content with relevance ranking, controlled spelling/acronym variants and Chinese search terms. Full identifiers resolve separately. view:summary (default) returns summaries; view:research returns multiple complete problem research views for the same query, including full statements, research history and references. Research pages fit whole problems within maxBytes (default 65536 UTF-8 bytes of the entire API JSON response, excluding MCP framing); no problem text is truncated. If the first problem cannot fit, response_budget_too_small reports minimumRequiredBytes and problemId. Default sort is relevance with text, otherwise catalog edit time (not solution date). Repeat filters, view and sort with nextCursor; limit and maxBytes may change. If the catalog changed, start again. total counts all matches; count is this page. Difficulty may be unrated; never infer an objective difficulty score from ordering. Catalog edit dates are not scientific resolution dates.", inputSchema: { type: "object", dependentSchemas: { maxBytes: { required: ["view"], properties: { view: { const: "research" } } } }, properties: { ...problemFilters, view: { enum: ["summary", "research"], default: "summary", description: "Choose summaries or complete research views; tool combinations depend on the task." }, maxBytes: { type: "integer", minimum: 16384, maximum: 1048576, description: "Only with view:research. Entire API JSON response budget in UTF-8 bytes; API default is 65536. Excludes HTTP headers, MCP framing and token accounting." }, limit: { type: "integer", minimum: 1, maximum: 200, default: 50 }, offset: { type: "integer", minimum: 0, description: "Legacy pagination; prefer cursor for change detection." }, cursor: { type: "string", minLength: 1, description: "nextCursor from the previous page; repeat filters, view and sort, omit offset. limit and maxBytes may change." }, sort: { enum: ["relevance", "edited", "title", "stale"], description: "edited: catalog TeX edit time; stale: service human-review age. Neither is the date a scientific problem was solved." } } }, call: async (a) => {
