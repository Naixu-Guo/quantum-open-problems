...
export const SERVER_INFO = { name: "quantum-open-problems", version: "1.2.0" };
...
export function createAdapter(serviceUrl: string, apiKey: string | null = null, optionsReadOnly = false) {
  ...
  async function http(method: "GET" | "POST", route: string, options: { body?: unknown; raw?: Uint8Array<ArrayBuffer>; headers?: Record<string, string> } = {}): Promise<{ status: number; body: unknown }> {
    ...
    const init: RequestInit = { method, headers, signal: AbortSignal.timeout(20_000), redirect: "error" };
  ...
  const TOOLS: Tool[] = [
    // Read
    { readOnly: true, name: "get_status", description: "Release and ledger sequence; problems counts permanent records, distinctQuestions counts published mathematical questions once across equivalent formulations. Each has Solved/Unsolved totals.", inputSchema: { type: "object", properties: {} }, call: () => http("GET", "/api/v1/status") },
    ...
    { readOnly: true, name: "search_problems", description: "Search indexed problem records. total is all matches; count is this page only (default 50). Repeat the same filters with nextOffset until it is null. Counts refer to records; get_status also provides distinct-question counts.", inputSchema: { type: "object", properties: { area: { type: "string", description: "Area label or slug, case-insensitive; e.g. Quantum Communication. See get_taxonomy." }, topic: { type: "string", description: "Topic label or slug, case-insensitive; e.g. Private capacity. See get_taxonomy." }, status: { enum: ["Unsolved", "Solved"] }, difficulty: { type: "string" }, text: { type: "string" }, limit: { type: "integer", minimum: 1, maximum: 200, default: 50 }, offset: { type: "integer", minimum: 0, default: 0 }, sort: { enum: ["title", "stale"], description: "stale: missing human-review dates first, then oldest review; ties by title and id. Not catalog edit age." }, includeCandidates: { type: "boolean" } } }, call: (a) => http("GET", `/api/v1/problems${query(a)}`) },
    { readOnly: true, name: "get_problem", description: "A problem with statement, clause statuses, references, comments, and decisions. Omits the duplicate authored record by default; use build_context for a token-bounded view.", inputSchema: { type: "object", required: ["id"], properties: { id: S.id, includeAuthoredRecord: { type: "boolean", default: false, description: "Include the large original catalog record only when its raw authored fields are needed." } } }, call: (a) => http("GET", `/api/v1/problems/${encodeURIComponent(str(a, "id"))}${query({ includeAuthoredRecord: a["includeAuthoredRecord"] })}`) },
    ...
    { readOnly: true, name: "build_context", description: "A bounded bundle for one problem under a token budget: statement, chosen clauses, accepted claims, tree, routes tried, references with notes, comments. Returns a bundle id to record in start_trajectory.", inputSchema: { type: "object", required: ["id"], properties: { id: S.id, clauseIds: { type: "array", items: S.clauseRef }, tokenBudget: { type: "integer", minimum: 200, default: 8000 } } }, call: (a) => http("GET", `/api/v1/problems/${encodeURIComponent(str(a, "id"))}/context${query({ clauses: Array.isArray(a["clauseIds"]) ? (a["clauseIds"] as string[]).join(",") : undefined, budget: a["tokenBudget"] })}`) },
    ...
    { readOnly: true, name: "claim_queue_item", description: "For verifier and triage agents: the oldest contribution waiting for review that is not the caller's own and that the caller has not reviewed, with the review packet fields.", inputSchema: { type: "object", properties: {} }, call: async () => {
  ...
  return { tools: optionsReadOnly ? TOOLS.filter(tool => tool.readOnly) : TOOLS, readResource, resources: RESOURCES, resourceTemplates: RESOURCE_TEMPLATES };
}
