...
export function createAdapter(serviceUrl: string, apiKey: string | null = null, optionsReadOnly = false) {
  ...
  const TOOLS: Tool[] = [
    ...
    { readOnly: true, name: "search_problems", description: "Search scientific content with relevance ranking, controlled spelling/acronym variants and Chinese search terms. Full identifiers resolve separately. Default sort is relevance with text, otherwise catalog edit time (not solution date). Repeat the same filters with nextCursor; if the catalog changed, start again. total counts all matches; count is this page. Difficulty may be unrated; never infer an objective difficulty score from ordering. For recently solved questions, filter Solved and read dated research evidence, not just catalogEditedAt.", inputSchema: { type: "object", properties: { ...problemFilters, limit: { type: "integer", minimum: 1, maximum: 200, default: 50 }, offset: { type: "integer", minimum: 0, description: "Legacy pagination; prefer cursor for change detection." }, cursor: { type: "string", minLength: 1, description: "nextCursor from the previous page; repeat the same filters and sort, omit offset." }, sort: { enum: ["relevance", "edited", "title", "stale"], description: "edited: catalog TeX edit time; stale: service human-review age. Neither is the date a scientific problem was solved." } } }, call: (a) => retrieval(`/api/v1/problems${query(a)}`, "qop-search/2") },
