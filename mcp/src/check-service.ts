...
import { AdapterError, CONTEXT_UPGRADE_MESSAGE, IDEMPOTENCY_UPGRADE_MESSAGE, REQUIRED_CONTEXT_SCHEMA_VERSION, REQUIRED_IDEMPOTENCY_VERSION, REQUIRED_RETRIEVAL_VERSION, RETRIEVAL_UPGRADE_MESSAGE, createAdapter, type Json } from "./adapter.ts";
...
export async function checkService(serviceUrl: string) {
  ...
  return { retrievalVersion: REQUIRED_RETRIEVAL_VERSION, serviceUrl: new URL(serviceUrl).origin, contextSchemaVersion: REQUIRED_CONTEXT_SCHEMA_VERSION, idempotencyVersion: REQUIRED_IDEMPOTENCY_VERSION };
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    const result = await checkService(process.env["QOP_SERVICE_URL"] ?? "http://127.0.0.1:8787");
    process.stdout.write(`Compatible API: ${result.serviceUrl} (${result.contextSchemaVersion}, ${result.idempotencyVersion}, ${result.retrievalVersion})\n`);
  } catch (error) {
