...
test("initialize, tools/list, and resources/templates/list describe the interface", async () => {
  ...
  assert.deepEqual(tools.result.tools.find((t: { name: string }) => t.name === "search_problems").inputSchema.properties.status.enum, ["Unsolved", "Solved"]);
...
test("read tools return records with ids and digests", async () => {
  ...
  const search = await tool("search_problems", { status: "Unsolved" });
  assert.ok(search.body.count >= 1);
  assert.ok(search.body.problems.every((problem: { status: string }) => problem.status === "Unsolved"));
