...
export function buildCompatibility({ write, records, payloads, apiIndex, legacy, config }) {
  ...
  write("api/v1/README.md", "# Static API compatibility\n\nHistorical problem URLs, JSON lookups, Markdown packets, release polling, snapshots, and the evidence feed are available here. Problem responses now declare `schema: qiqcop-zoo/problem/3`, use permanent op IDs and the binary statuses `Solved` and `Unsolved`, and match `/api/problems/`. Read this schema before migrating an old client; the former v1 payload shape is retired. Unmapped historical entries return `qiqcop-zoo/archived-entry/1` with an archive link rather than an inferred identity. Evidence timestamps are catalog edit times. The static write contract is retired; its schema rejects payloads and directs authors to the service or catalog PR workflow.\n");
}
