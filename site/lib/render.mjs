  ...
  const mcpConfig = JSON.stringify({
    mcpServers: {
      "quantum-open-problems": {
        ...
        env: { QOP_SERVICE_URL: "http://localhost:8787" }
      }
    }
  }, null, 2);
  ...
  const body = `
    <section class="section-shell about">
      ...
      <div class="prose">
        ...
        <p>The current setup runs a local copy of the catalog. You need Git, Node.js 22.13 or later, and an MCP client that supports local <code>stdio</code> servers. Reading problems requires no API key.</p>
        <ol>
          <li><strong>Start the service.</strong> Run these commands in a terminal and leave it open:
            <div class="copy-block no-math"><pre id="mcp-setup">${escape(mcpSetup)}</pre><button class="copy-button" type="button" data-copy="mcp-setup" aria-label="Copy service setup commands">Copy</button></div>
          </li>
        ...
        <p>See the <a href="${config.repositoryUrl}/blob/${config.branch}/mcp/README.md" rel="noreferrer">MCP setup and tool guide</a> for connection troubleshooting and authenticated research contributions. For direct downloads, the <a href="${root}api/index.json">JSON catalog</a>, <a href="${root}api/tags.json">taxonomy</a>, and <a href="${root}llms.txt">agent guide</a> are also available.</p>
