...
The catalog integration in [CATALOG_INTEGRATION.md](CATALOG_INTEGRATION.md)
supersedes the seed-data, identifier, taxonomy, and research-status details
below: the maintained JSON database is authoritative, both identifiers are
retained, and research status is exactly `Solved` or `Unsolved`.
