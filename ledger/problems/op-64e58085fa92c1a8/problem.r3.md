...
Ahmed and Smith rule out finite-dimensional attainment on a rank-two Bell-diagonal subfamily (Theorem 2 of a 2026 preprint) [AS26](https://arxiv.org/abs/2609.14031v1). Set $p_I=(1+c)/2$, $p_Z=(1-c)/2$, and $p_X=p_Y=0$, where $0<c<1$, and write $\rho_c=\rho_{\mathbf p}$. Let $E_D(\rho_c)$ be the minimum in Eq. (2) with $\dim E\leq D$. For every positive integer $D$,
...
Equation (4) implies that no finite extension dimension attains the infimum, even on this subfamily. It does not determine the value or provide a certified approximation error.

Historical GitHub report (2026-09-16): Naixu-Guo reported the finite-dimensional nonattainment result for squashed entanglement in the Ahmed–Smith preprint. [Pull request #70](https://github.com/Naixu-Guo/quantum-open-problems/pull/70).
...
Finite-dimensional nonattainment does not exclude certified approximation: $E_{\mathrm{sq}}=\inf_D E_D$ still guarantees a finite-dimensional $\varepsilon$-approximation for every $\varepsilon>0$. The new obstruction provides neither a computable dimension bound nor a convergence rate [AS26](https://arxiv.org/abs/2609.14031v1).
...
**AS26** R. G. Ahmed and G. Smith, “No cardinality bound for squashed entanglement,” arXiv preprint (2026). [arXiv:2609.14031v1](https://arxiv.org/abs/2609.14031v1).
