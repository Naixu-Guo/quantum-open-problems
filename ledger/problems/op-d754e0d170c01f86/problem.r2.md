...
The essential constraint is uniform channel smoothing in diamond norm. An AEP obtained by smoothing each output state separately, or by optimizing only over unentangled inputs, does not imply Eq. (3). A positive solution would also resolve the conditional equality posed in [the parallel versus nested-adaptive superchannel-discrimination problem](https://qiqc-op.com/problem/op_a4600b38b94042a8/).
