---
id: "01M20X12BN1JR5S14659HFQ1PB"
schemaVersion: "1.0"
createdBy: "01M1VDQX7KEACQQ8KY2HZ5DTFK"
createdAt: "2026-09-08T16:18:42.677Z"
type: "Statement"
supersedes: "01M20THHNMGH4PQTAEC9B430NH"
problemId: "01M208CJ9MKVF4X0B83M9V7QZM"
version: 4
digest: "sha256:75b2f752abb45ae1a69b25927a12926d94a0f0925996d11daa9d8221f6f36878"
clauses: [{"id":"main","label":"Problem","text":"Does the exact stabilizer rank of the tensor powers of the single-qubit\nmagic state $|T\\rangle$ grow polynomially in the number of copies, or is\nit not polynomially bounded?  The state is\n\\begin{equation}\n  |T\\rangle := 2^{-1/2}\\bigl(|0\\rangle + e^{i\\pi/4}|1\\rangle\\bigr),\n  \\label{eq:srtk-t-state}\n\\end{equation}\nand, for an $n$-qubit pure state $|\\psi\\rangle$, its exact stabilizer rank\nis\n\\begin{equation}\n  \\chi(\\psi) := \\min\\Bigl\\{r : |\\psi\\rangle = \\sum_{j=1}^{r} c_j\\,|\\varphi_j\\rangle\n  \\text{ for some } c_j \\in \\mathbb{C} \\text{ and } n\\text{-qubit stabilizer states }\n  |\\varphi_j\\rangle\\Bigr\\},\n  \\label{eq:srtk-rank-def}\n\\end{equation}\nwhere an $n$-qubit stabilizer state is the common $+1$ eigenstate of $n$\nindependent commuting Hermitian Pauli operators.  Which of the two alternatives holds for\n$\\chi\\bigl(|T\\rangle^{\\otimes n}\\bigr)$: does there exist a constant $k$\nwith $\\chi\\bigl(|T\\rangle^{\\otimes n}\\bigr) \\le n^{k}$ for all sufficiently large $n$, or is\n$\\chi\\bigl(|T\\rangle^{\\otimes n}\\bigr)$ not bounded by any polynomial in\n$n$?  The best current bounds are\n\\begin{equation}\n  \\Omega\\!\\left(\\frac{n^{2}}{\\operatorname{polylog} n}\\right) \\le\n  \\chi\\bigl(|T\\rangle^{\\otimes n}\\bigr) \\le O\\bigl(2^{\\alpha n}\\bigr),\n  \\qquad \\alpha = \\tfrac{1}{4}\\log_{2} 3 \\le 0.3963,\n  \\label{eq:srtk-bounds}\n\\end{equation}\nwhich leave both alternatives open.  A sharper sub-question is whether the\nexponential rate $\\gamma = \\lim_{n\\to\\infty} \\frac{1}{n}\\log_{2}\n\\chi\\bigl(|T\\rangle^{\\otimes n}\\bigr)$, which exists by sub-multiplicativity\nof the rank under tensor product and Fekete's lemma, is strictly positive,\nsince Eq.~\\eqref{eq:srtk-bounds} only pins $\\gamma$ to the interval\n$[0, 0.3963]$.  The small-copy frontier of the quantity in\nEq.~\\eqref{eq:srtk-rank-def} on copies of the state in\nEq.~\\eqref{eq:srtk-t-state} is also open: the exact values $\\chi\\bigl(|T\\rangle^{\\otimes 2}\\bigr) = 2$\nand $\\chi\\bigl(|T\\rangle^{\\otimes 3}\\bigr) = 3$ are proved, while at four\nthrough seven copies only upper bounds are known,\n$\\chi\\bigl(|T\\rangle^{\\otimes 4}\\bigr) \\le 4$,\n$\\chi\\bigl(|T\\rangle^{\\otimes 5}\\bigr) \\le 6$,\n$\\chi\\bigl(|T\\rangle^{\\otimes 6}\\bigr) \\le 6$, and\n$\\chi\\bigl(|T\\rangle^{\\otimes 7}\\bigr) \\le 12$, and $\\chi\\bigl(|T\\rangle^{\\otimes 8}\\bigr) \\le 12$ (Table~1 of the same source), the four-copy bound having\nsince been certified as the exact value $\\chi\\bigl(|T\\rangle^{\\otimes\n4}\\bigr) = 4$ by an exhaustive computer search; determine the exact values\nat $t = 5$, $6$, $7$, and $8$.","kind":"decision","resolutionCriteria":"Resolve the authored problem statement as stated.","supersedesClauseId":null,"quantity":null}]
---
Does the exact stabilizer rank of the tensor powers of the single-qubit magic state $|T\rangle$ grow polynomially in the number of copies, or is it not polynomially bounded? The state is

$$
|T\rangle := 2^{-1/2}\bigl(|0\rangle + e^{i\pi/4}|1\rangle\bigr),
\tag{1}
$$

and, for an $n$-qubit pure state $|\psi\rangle$, its exact stabilizer rank is

$$
\chi(\psi) := \min\Bigl\{r : |\psi\rangle = \sum_{j=1}^{r} c_j\,|\varphi_j\rangle
 \text{ for some } c_j \in \mathbb{C} \text{ and } n\text{-qubit stabilizer states }
 |\varphi_j\rangle\Bigr\},
\tag{2}
$$

where an $n$-qubit stabilizer state is the common $+1$ eigenstate of $n$ independent commuting Hermitian Pauli operators. Which of the two alternatives holds for $\chi\bigl(|T\rangle^{\otimes n}\bigr)$: does there exist a constant $k$ with $\chi\bigl(|T\rangle^{\otimes n}\bigr) \le n^{k}$ for all sufficiently large $n$, or is $\chi\bigl(|T\rangle^{\otimes n}\bigr)$ not bounded by any polynomial in $n$? The best current bounds are

$$
\Omega\!\left(\frac{n^{2}}{\operatorname{polylog} n}\right) \le
 \chi\bigl(|T\rangle^{\otimes n}\bigr) \le O\bigl(2^{\alpha n}\bigr),
 \qquad \alpha = \tfrac{1}{4}\log_{2} 3 \le 0.3963,
\tag{3}
$$

which leave both alternatives open. A sharper sub-question is whether the exponential rate $\gamma = \lim_{n\to\infty} \frac{1}{n}\log_{2}
\chi\bigl(|T\rangle^{\otimes n}\bigr)$, which exists by sub-multiplicativity of the rank under tensor product and Fekete’s lemma, is strictly positive, since Eq. (3) only pins $\gamma$ to the interval $[0, 0.3963]$. The small-copy frontier of the quantity in Eq. (2) on copies of the state in Eq. (1) is also open: the exact values $\chi\bigl(|T\rangle^{\otimes 2}\bigr) = 2$ and $\chi\bigl(|T\rangle^{\otimes 3}\bigr) = 3$ are proved, while at four through seven copies only upper bounds are known, $\chi\bigl(|T\rangle^{\otimes 4}\bigr) \le 4$, $\chi\bigl(|T\rangle^{\otimes 5}\bigr) \le 6$, $\chi\bigl(|T\rangle^{\otimes 6}\bigr) \le 6$, and $\chi\bigl(|T\rangle^{\otimes 7}\bigr) \le 12$, and $\chi\bigl(|T\rangle^{\otimes 8}\bigr) \le 12$ (Table 1 of the same source), the four-copy bound having since been certified as the exact value $\chi\bigl(|T\rangle^{\otimes
4}\bigr) = 4$ by an exhaustive computer search; determine the exact values at $t = 5$, $6$, $7$, and $8$.
