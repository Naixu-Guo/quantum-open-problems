...
% is one "references" entry, Field and Topic are the "fields" and "topics"
% arrays, and the remaining subsections are the record keys of the same name.
%
% Rules:
...
% 8. Keep Field, Topic, and ID as the final three subsections, in that order.
%    Assign one or two fields from the "fields" list and one to five topics
%    from the "topics" list of database/tags.json, separated by semicolons.
%    (Older files carry a single Tag subsection instead; it is still read.)
...
Name.} only with permission for named credit on this problem; otherwise
write \texttt{unknown}.
...
% One or two names from the "fields" list of database/tags.json, exactly as
% written, separated by semicolons: the broad research areas of the problem.
Canonical field

\subsection*{Topic}

% One to five names from the "topics" list of database/tags.json, exactly as
% written, separated by semicolons: the specific objects and techniques.
Canonical topic one; Canonical topic two
