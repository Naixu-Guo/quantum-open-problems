...
these cases.  Eq.~\eqref{eq:qmds-huber-grassl-bound} exceeds $b(q,d)$ by one
for $d=4$ with $q$ odd and by $d-3$ for every $d\geq5$; Huber and Grassl
state that Conjecture~12 is otherwise unresolved for $d>3$.  For stabilizer
codes the odd-$q$, $d=4$ window is closed by the orthogonal-array bound of
Eq.~\eqref{eq:qmds-bush-odd} below.

  \item For $k\geq1$ the stabilizer of a QMDS code is itself an additive MDS
code: by Theorem~2 of \sourcecite{ref:qmds-rains}{Rai99}, restated as
Lemma~59 and Corollary~60 of \sourcecite{ref:qmds-ketkar}{KKKS06}, every
QMDS code $[[n,n-2(d-1),d]]_q$ with $k\geq1$ is pure to weight $n-d+2$, so
Lemma~61 of \sourcecite{ref:qmds-ketkar}{KKKS06} identifies its stabilizer
with an additive code $C\subseteq\mathbb{F}_{q^{2}}^{\,n}$, $|C|=(q^{2})^{d-1}$,
of minimum Hamming distance $n-d+2$.  A code, linear or not, with $s^{t}$
words and minimum distance $n-t+1$ over an alphabet of size $s$ is an
orthogonal array of strength $t$ and index one: two words agreeing on $t$
coordinates would be at distance at most $n-t$, so every $t$-tuple of
symbols occurs exactly once in every set of $t$ coordinates.  Hence $C$ is
an orthogonal array of strength $d-1$ and index one on $q^{2}$ symbols.
Bush's bound for such arrays, \sourcecite{ref:qmds-bush}{Bus52}, allows at
most $s+t-1$ coordinates when $s$ is even and $t\leq s$, and at most $s+t-2$
when $s$ is odd and $3\leq t\leq s$.  With $s=q^{2}$ and $t=d-1$ this gives
\begin{equation}
  n\leq q^{2}+d-3
  \qquad(q\text{ odd},\ 4\leq d\leq q^{2}+1),
  \label{eq:qmds-bush-odd}
\end{equation}
one less than Eq.~\eqref{eq:qmds-huber-grassl-bound}, while for even $q$ it
reproduces Eq.~\eqref{eq:qmds-huber-grassl-bound}; Theorem~63 of
\sourcecite{ref:qmds-ketkar}{KKKS06} obtains only the latter value from the
MacWilliams identities.  For $d=4$ and odd $q$, Eq.~\eqref{eq:qmds-bush-odd}
reads $n\leq q^{2}+1=b(q,4)$, so Eq.~\eqref{eq:qmds-length} is impossible;
together with the previous item the conjecture therefore holds for every
$q$ when $d\leq4$.  This consequence is a deduction from the cited results
and is not stated in \sourcecite{ref:qmds-huber-grassl}{HG20}; it uses the
stabilizer structure and does not extend to nonstabilizer QMDS codes.
...
powers.  For $q=4$ they show, Section~7 there, that a stabilizer
counterexample would have to come from an additive
$(18,16^{k},19-k)_{16}$ MDS code that is not linear over $\mathbb{F}_{4}$,
since additive MDS codes over $\mathbb{F}_{16}$ that are linear over
$\mathbb{F}_{4}$ obey the MDS conjecture.
...
\sourcecite{ref:qmds-ball-centelles-huber}{BCH23}.
  ...
  \item Historical GitHub report (2026-09-13): bikunli reported the deduction of odd-alphabet stabilizer-QMDS length bounds from orthogonal-array bounds. \href{https://github.com/Naixu-Guo/quantum-open-problems/pull/58}{Pull request \#58}. Follow-up: \href{https://github.com/Naixu-Guo/quantum-open-problems/pull/58#issuecomment-5669683576}{bikunli, 2026-09-14}.
\end{itemize}
...
\href{https://arxiv.org/abs/quant-ph/0508070}{arXiv:quant-ph/0508070}.
  ...
  \item[\textup{[Rai99]}]\label{ref:qmds-rains}
  E. M. Rains, ``Nonbinary quantum codes,'' \emph{IEEE Transactions on
Information Theory} \textbf{45}, 1827--1832 (1999).
...
\href{https://arxiv.org/abs/2007.05992}{arXiv:2007.05992}.
  ...
  \item[\textup{[Bus52]}]\label{ref:qmds-bush}
  K. A. Bush, ``Orthogonal arrays of index unity,'' \emph{The Annals of
Mathematical Statistics} \textbf{23}, 426--434 (1952).
\href{https://doi.org/10.1214/aoms/1177729387}{doi:10.1214/aoms/1177729387}.
\end{enumerate}

\subsection*{Comment}

Any unresolved case must have $d\geq5$.  In this range, for odd $q$,
Eq.~\eqref{eq:qmds-bush-odd} leaves the window $q^{2}+1<n\leq q^{2}+d-3$, and
for even $q$, Eq.~\eqref{eq:qmds-huber-grassl-bound} leaves
$q^{2}+1<n\leq q^{2}+d-2$.  These are necessary candidate ranges, not
claims that every parameter choice remains open.  The bounds in Progress
exclude $n>b(q,d)$ for $d\leq4$.  For even $q$ and $d=4$, the allowed
exception $n=q^{2}+2=b(q,4)$ is attained by the known family in Progress;
it is not a counterexample.  Only the alphabets in
Eq.~\eqref{eq:qmds-small-alphabets} are settled for every $d$.
A stabilizer counterexample would give, through
