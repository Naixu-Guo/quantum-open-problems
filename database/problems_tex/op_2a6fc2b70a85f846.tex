...
The unresolved cases are $d\geq5$.  For odd $q$,
Eq.~\eqref{eq:qmds-bush-odd} leaves the window $q^{2}+1<n\leq q^{2}+d-3$, and
for even $q$, Eq.~\eqref{eq:qmds-huber-grassl-bound} leaves
$q^{2}+1<n\leq q^{2}+d-2$; both windows are empty for $d\leq4$, and only the
alphabets in Eq.~\eqref{eq:qmds-small-alphabets} are settled for every $d$.
