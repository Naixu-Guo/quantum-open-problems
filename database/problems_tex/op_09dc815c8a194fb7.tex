...
$E_D(\rho_{\mathrm{as}}^{(d)})$ exactly for every $d\ge3$.  A complete answer
...
\begin{itemize}
  ...
  outcomes, each of unconditional probability $2/(d(d-1))$, occur with total
  probability
  \begin{equation}
...
falls below it by a definite factor is open.  Determining the existence and value of this limit is a secondary
asymptotic question, not a complete answer to the exact finite-dimensional
problem.  A positive LOCC--PPT gap alone does not determine the limit: a
gap of order $1/d^2$ leaves the leading constant unchanged.  Even a gap
bounded below by $c/d$ for a fixed $c>0$ would only exclude the PPT leading
constant, without establishing existence or the exact value of the LOCC
limit.  A complete solution requires matching LOCC achievability and
converse bounds for every finite $d\ge3$; the PPT side itself is closed.  This record asks for the
