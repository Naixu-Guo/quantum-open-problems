...
\begin{itemize}
  ...
  parties measure in the computational basis and keep exactly the joint
  outcomes $(i,j)$, $i\ne j$, whose two letters lie in the same pair.  On
  the two-dimensional span of $\lvert ij\rangle$ and $\lvert ji\rangle$ the
  state in Eq.~\eqref{eq:09dc-antisymmetric-state} is the singlet, so every
  kept outcome is one ebit, and the $2\lfloor d/2\rfloor$ ordered same-pair
