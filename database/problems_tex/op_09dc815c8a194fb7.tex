...
\begin{itemize}
  ...
  constant, making asymptotic manipulation of the state irreversible at
  every $d$; the exact distillable entanglement is left open
...
while the achievability side is known only as strict positivity, so neither
the existence nor the value of
$\lim_{d\to\infty}d\,E_D(\rho_{\mathrm{as}}^{(d)})$ is established.  A proof that the LOCC and PPT rates differ on this family, or
an exact evaluation of the Rains bound of $\rho_{\mathrm{as}}^{(d)}$ with a
matching achievable protocol, would settle one of the posed alternatives.
