...
$2^{n/2+o(n)}$-time algorithm for $\sqrt{n}$-SVP and $\sqrt{n}$-Hermite
SVP, together with broader time--approximation tradeoffs.  Polynomial
approximation factors are therefore accessible significantly faster than
exact SVP in some regimes, but not in polynomial time
...
The related LWE and Dihedral Hidden Subgroup Problem records are given
related problem links (Progress above).
