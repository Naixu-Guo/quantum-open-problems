...
\begin{itemize}
  ...
  \item Intermediate rank, lower bounds. Any nonadaptive single-copy scheme needs $\Omega(r^2 d/\epsilon^2)$ copies for states of rank $r$ (Theorem~4.11 of \sourcecite{ref:df68-lowe-nayak}{LN25}), which gives the bound $1+2\alpha$ for nonadaptive algorithms. For adaptive single-copy schemes the rank-$r$ rate is unknown, since the adaptive bound $\Omega(d^3/\epsilon^2)$ of \sourcecite{ref:df68-chen}{CHL+23} covers full rank only, a gap recorded in Section~1.2 of \sourcecite{ref:df68-acharya-stoc}{ADLY25}. In the adaptive model \eqref{eq:df68-known-bounds} therefore rests on the bound $\tilde\Omega(dr/\epsilon^2)$ for measurements entangled across copies \sourcecite{ref:df68-haah}{HHJ+17}. No lower bound specific to Pauli measurements depends on $r$. The hard instances behind the full-rank bounds are small perturbations of the maximally mixed state (Definition~4.1 of \sourcecite{ref:df68-acharya-stoc}{ADLY25}) and say nothing about low-rank states.

  \item Two-outcome Pauli measurements. When each copy is used to measure one Pauli observable, so that only the product of the outcomes in \eqref{eq:df68-pauli-povm} is kept, $O(r^2d^2\log d/\epsilon^2)$ copies suffice (Theorem~2 of \sourcecite{ref:df68-flammia}{FGLE12}), $\Omega(r^2d^2/\log d)$ copies are necessary at constant $\epsilon$ even for adaptive sequences of observables (Theorem~6 of \sourcecite{ref:df68-flammia}{FGLE12}), and $\Omega(r^2d^2/\epsilon^2)$ copies are necessary for nonadaptive measurements with a constant number of outcomes \sourcecite{ref:df68-lowe-nayak}{LN25}. This settles the two-outcome model at $\tilde\Theta(r^2 4^N/\epsilon^2)$, which is $16^N$ at full rank against $10^N$ for \eqref{eq:df68-pauli-povm}. The upper bound transfers to \eqref{eq:df68-pauli-povm}. The lower bounds do not.
\end{itemize}
...
The open range is $0<\alpha<1$. There is evidence that neither side of \eqref{eq:df68-known-bounds} is tight. The projected least squares bound misses the truth by an exponential factor at both endpoints, and the lower bound $1+2\alpha$ holds for every nonadaptive single-copy scheme, whereas at $\alpha=1$ Pauli measurements already lose the factor $(10/8)^N$ against the best single-copy scheme. Whether $c$ is convex or piecewise linear is not known. To give concrete numbers, for nonadaptive algorithms at $r=\sqrt d$ the two sides of \eqref{eq:df68-known-bounds} correspond to $4^N/\epsilon^2$ copies and, up to the factor $N$, to $6^N/\epsilon^2$ copies.

Nearby questions remain open as well. In the adaptive model the lower bound for intermediate rank is only $1+\alpha$, because no adaptive single-copy lower bound is known beyond full rank. At full rank the factor $\sqrt N$ between $10^N/\epsilon^2$ and $10^N/(\sqrt N\,\epsilon^2)$ remains, and \sourcecite{ref:df68-acharya-single-qubit}{ADLY25b} asks whether $10^N$ is the sample complexity of Pauli measurements. The same question with infidelity in place of \eqref{eq:df68-error} is listed with the low-rank extension in \sourcecite{ref:df68-acharya-stoc}{ADLY25}. The two-outcome model of the last Progress item is settled and is a different problem from \eqref{eq:df68-pauli-povm}.
