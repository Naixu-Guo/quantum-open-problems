...
\begin{itemize}
  ...
  $\mathsf{QMA}^{+}(2) = \mathsf{NEXP}$ and showed that
  $\mathsf{QMA}^{+}(2)$ with completeness--soundness gap at least
  $3/4 + 1/\operatorname{poly}(n)$ is contained in ordinary
  $\mathsf{QMA}(2)$, so that $\mathsf{QMA}(2) = \mathsf{NEXP}$ would follow
  if the gap of the non-negative-amplitude protocol could be strongly
  amplified, a step their Section~8 leaves unresolved: within
  $\mathsf{QMA}^{+}(2)$ they achieve only mild amplification, to gap $2/9$
  from any inverse-polynomial gap and to $1 - e^{-\operatorname{poly}(n)}$
  only from a gap of at least $7/8$.  For $\mathsf{QMA}(2)$ itself only the
  trivial containments of Eq.~\eqref{eq:qma2-containment} are known
  ...
  \item Historical GitHub report (2026-09-08): Chengkai-Zhu discussed the scope of two-prover complexity results and the gap conditions of the non-negative-amplitude variant. \href{https://github.com/Naixu-Guo/quantum-open-problems/pull/18}{Pull request \#18}. Follow-up: \href{https://github.com/Naixu-Guo/quantum-open-problems/pull/18#issuecomment-5585670052}{Naixu-Guo, 2026-09-08}.
\end{itemize}
...
The zoo's neighboring records pose different questions: ``\href{https://qiqc-op.com/problem/op_bca77ec42ddd1d5c/}{The quantum PCP conjecture}'' asks for QMA-hardness of constant-relative-gap local
Hamiltonians on a promise problem, and ``\href{https://qiqc-op.com/problem/op_ec184fc49232a9c0/}{Constant trace-distance separability testing}'' relates its promise gap to estimating
