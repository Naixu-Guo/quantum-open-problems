...
Bravyi, Browne, Calpin, Campbell, Gosset, and Howard explicitly pose the
asymptotic scaling of $\chi\bigl(|T\rangle^{\otimes n}\bigr)$, stating
that polynomial scaling would entail complexity-theoretic heresies such as
...
bounds \sourcecite{ref:srtk-bbc19}{BBC+19}; the quantity
$\chi\bigl(|T\rangle^{\otimes n}\bigr)$ and its use for classical
simulation were introduced by Bravyi, Smith, and Smolin
\sourcecite{ref:srtk-bss16}{BSS16}.
...
\begin{itemize}
  ...
  powers of a single-qubit magic state, conjecturing
  $\chi\bigl(|T\rangle^{\otimes 6}\bigr) = 7$
  \sourcecite{ref:srtk-bss16}{BSS16}; the sampling algorithm of Bravyi and
  ...
  of exact and approximate stabilizer rank and confirmed
  $\chi\bigl(|T\rangle^{\otimes t}\bigr) = t$ for $2 \le t \le 4$, with
  the numerical value $\chi\bigl(|T\rangle^{\otimes 7}\bigr) = 12$ marking
  the transition from linear to exponential scaling
  ...
  O\bigl(2^{\alpha n}\bigr)$, improving the previous
  ...
  closed-form four-copy decomposition of the T-type orbit merely matches
  the exponent $\alpha$ of Eq.~\eqref{eq:srtk-bounds}, so neither side of
  the qubit question moves \sourcecite{ref:srtk-lr26}{LR26}.
\end{itemize}
...
An explicit polynomial upper bound would give, by gadgetizing every T gate
through state injection into a Clifford circuit acting on
$|T\rangle^{\otimes m}$, a polynomial-time classical strong-simulation
algorithm for all Clifford+T circuits with polynomially many T gates, and by
Theorem~1.6 of \sourcecite{ref:srtk-mt24}{MT24} would imply
$\mathsf{P}^{\#\mathsf{P}} \subseteq \mathsf{P}/\mathsf{poly}$, so it is
believed false.  Any super-polynomial lower bound, even $n^{2+\varepsilon}$
for some $\varepsilon > 0$, would settle the conjectured super-polynomial
growth and be the first unconditional bound beyond the
