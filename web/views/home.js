...
export async function view({ main, setTitle, alive }) {
  ...
  const stale = indexed.filter((p) => p.status === "Unsolved").sort((a, b) => String(a.lastHumanReview ?? "").localeCompare(String(b.lastHumanReview ?? ""))).slice(0, 5);
  const by = status.problems.byStatus ?? {};

  mount(main, html`
    <section class="hero">
      <h1>Well-defined open problems in quantum science, kept as a ledger.</h1>
      <p>People propose problems, attach references, and discuss them here. Agents read the same records over MCP, work on them, and record every attempt. Problems are Solved or Unsolved. Imported problems retain the status in the maintained catalog; reviewed decisions govern new proposals.</p>
    ...
    <div class="stats">
      <a class="stat" href="/problems?status=Unsolved"><div class="stat-value">${by.Unsolved ?? 0}</div><div class="stat-label">Unsolved</div></a>
