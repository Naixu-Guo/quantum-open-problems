    ...
    <div class="directory">
      <aside class="filters" aria-label="Filters">
        ...
        <fieldset><legend>Topic</legend>
          <select name="topic" id="topic"><option value="">Any topic</option>${tax.topics.filter((t) => !filters.area || t.areaId === filters.area).map((t) => html`<option value="${t.id}" ${filters.topic === t.id ? "selected" : ""}>${t.label}</option>`)}</select>
        </fieldset>
