...
const MATH = /\$\$[\s\S]+?\$\$|\\\[[\s\S]+?\\\]|\\\([\s\S]+?\\\)|\\begin\{([a-z*]+)\}[\s\S]*?\\end\{\1\}|(?<![\\$\w])(\$(?:\\.|[^$\\])+?\$)(?![\d$])/g;
/** `[label](url)`; the URL may carry escaped characters and one level of balanced parentheses. */
const LINK = /\[([^\]\n]+)\]\(((?:\\.|[^\s()\\]|\([^\s()]*\))+)\)/g;
// The first opening delimiter decides: math may contain brackets, and a link label may
// contain math. Lifting every link first could consume an interval and its next citation.
const MATH_OR_LINK = new RegExp(`${MATH.source}|${LINK.source}`, "g");
...
export function inline(text, { linkable = true } = {}) {
  ...
  const liftMath = (whole, _env, dollars) => {
    if (dollars === undefined) return math(whole);
    return /^\s|\s$/.test(dollars.slice(1, -1)) ? whole : math(dollars);
  };
  let s = codeSpans(String(text), keep);
  if (linkable) {
    s = s.replace(MATH_OR_LINK, (whole, env, dollars, label, url) => {
      if (label === undefined) return liftMath(whole, env, dollars);
